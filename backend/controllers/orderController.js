...
export const saveOrder = catchAsyncErrors(async (req, res, next) => {
  ...
  let { cartItems, orderType } = req.body;
  ...
  if (typeof cartItems === "string") {
    try {
      ...
      return next(new ErrorHandler("Invalid cart items.", 400));
    }
  }
  ...
  if (!email || !phone || !orderType || Number.isNaN(parsedTotal) || parsedTotal <= 0 || cartItems.length === 0) {
    return next(new ErrorHandler("Please fill all required fields.", 400));
  }
  if (cartItems.some((item) => !item.name || !item.image || !item.size)) {
    return next(new ErrorHandler("Some cart items are missing details. Add them to the cart again.", 400));
  }
  ...
  if (!date || !time || !people || !tableNumber) {
    return next(new ErrorHandler("Please fill all required fields.", 400));
  }

  const seated = await Order.findOne({
    orderType: "Dine In",
    date,
    time,
  ...
  if (seated || reserved) {
    return next(new ErrorHandler("This table is already taken for that date and time.", 409));
  }
